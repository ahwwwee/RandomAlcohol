...
public class AlcoholController {
	...
	public Random rand;
	public static List<Float> typesOptions;
	float alcohol;
	...
	public static MongoOperations getMongoOperations(String type) throws Exception {
		...
		if (type == null) {
			...
			numberLimit = (int) mongoOperation.count(Query.query(Criteria.where("type").is(type)), "Alcohol");
		}
	...
	public @ResponseBody String random(@RequestBody String type) throws Exception {
		...
		Alcohol alco = mongoOperation.findOne(Query.query(Criteria.where("type").is(type)).skip(random), Alcohol.class,
				"Alcohol");
		return mapper.writeValueAsString(random + " " + numberLimit);
	}
